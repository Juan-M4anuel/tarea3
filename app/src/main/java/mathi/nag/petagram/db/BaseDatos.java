...
public class BaseDatos extends SQLiteOpenHelper {
    ...
    public ArrayList<Mascota> obtenerTodasLasMascotas(){
        ...
        while (registros.moveToNext()){
            ...
            String queryLikes = "SELECT COUNT(" +ConstantesBaseDatos.TABLE_LIKES_MASCOTAS_NUMERO_LIKES + ") " + " FROM " + ConstantesBaseDatos.TABLE_LIKES_MASCOTAS +
    ...
    public String ObtenerLikesMascota(Mascota mascota){
        ...
        String query = "SELECT COUNT("+ConstantesBaseDatos.TABLE_LIKES_MASCOTAS_NUMERO_LIKES +")" + " FROM " + ConstantesBaseDatos.TABLE_LIKES_MASCOTAS +
                " WHERE " + ConstantesBaseDatos.TABLE_LIKES_MASCOTAS_ID_MASCOTA + " = " +  mascota.getId();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor registros = db.rawQuery(query, null);

        if (registros.moveToNext()){
            likes = registros.getString(0);
        }
        db.close();
        return  likes;
    }
